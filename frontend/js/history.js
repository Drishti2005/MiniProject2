...
function populateStats(sessions, statsEl) {
    ...
    const recentLabel = most_recent
        ? formatDate(most_recent.started_at || most_recent.created_at).split(',')[0]
        : '—';
...
function buildSessionCard(session) {
    ...
    const duration = session.ended_at
        ...
        : 'Ongoing';
    const preview    = session.title || 'Medical Session';
    const isEnded    = !!session.ended_at;
    const hasSummary = !!session.has_summary;
    const phraseCount = session.transcript_count || 0;
    const langNames  = { en:'English', hi:'Hindi', es:'Spanish', fr:'French', de:'German', zh:'Chinese', ar:'Arabic', bn:'Bengali', ta:'Tamil', te:'Telugu', mr:'Marathi', gu:'Gujarati' };
    const langLabel  = langNames[session.language] || session.language || 'English';
        ...
        <div class="session-card-body">
            <div class="session-card-top">
                ...
                <div style="display:flex;gap:0.4rem;flex-wrap:wrap;justify-content:flex-end">
                    ...
                    ${phraseCount === 0 ? '<span class="badge" style="background:var(--surface-container);color:var(--on-surface-variant)">Empty</span>' : ''}
                </div>
            </div>
            ...
            <div class="session-card-summary">${escapeHtml(preview)}</div>
        </div>
...
function renderSessionDetail(container, apiResponse) {
    // API returns { session: {...}, transcript: [...], simplifications: [...], summary: {...} }
    const sessionMeta   = apiResponse.session      || apiResponse;
    const transcriptArr = apiResponse.transcript   || [];
    const simpArr       = apiResponse.simplifications || [];
    const summary       = apiResponse.summary      || null;

    const date     = formatDate(sessionMeta.started_at || sessionMeta.created_at);
    const duration = sessionMeta.ended_at
        ? formatDuration(sessionMeta.started_at || sessionMeta.created_at, sessionMeta.ended_at)
        : 'Ongoing';
        ...
        <div class="detail-card">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                ...
                <div>
                    ...
                    <div style="font-size:0.72rem;font-family:monospace;color:var(--on-surface-variant)">${escapeHtml(String(sessionMeta.id || '—').substring(0, 24))}…</div>
                </div>
                <div>
                    ...
                    <span class="badge badge-${sessionMeta.ended_at ? 'ended' : 'active'}">${sessionMeta.ended_at ? 'Completed' : '● Active'}</span>
                </div>
            </div>
        </div>`;

    // ── Transcript card — join chunk objects into readable text
    const transcriptText = transcriptArr.length > 0
        ? transcriptArr.map(c => typeof c === 'string' ? c : (c.text || '')).filter(Boolean).join(' ')
        : '';

    html += `
        <div class="detail-card">
            <div class="detail-card-title">Full Transcript (${transcriptArr.length} phrase${transcriptArr.length !== 1 ? 's' : ''})</div>
            ${transcriptText
    ...
    if (simpArr.length > 0) {
        const termsHtml = simpArr.map(s => `
            <div class="term-item">
                <span class="term-chip">${escapeHtml(s.term || '')}</span>
                <p class="term-explanation">${escapeHtml(s.explanation || '')}</p>
            </div>`).join('');
        html += `
            <div class="detail-card">
                <div class="detail-card-title">AI-Simplified Terms (${simpArr.length})</div>
    ...
    if (summary) {
        ...
        if (summary.medications && summary.medications.length) summaryHtml += `
            <div class="summary-section">
        ...
        if (summary.instructions && summary.instructions.length) summaryHtml += `
            <div class="summary-section">
        ...
        if (summary.follow_up) summaryHtml += `
            <div class="summary-section">
                <div class="summary-section-label">Follow-up</div>
                <p class="summary-text">${escapeHtml(summary.follow_up)}</p>
            </div>`;

        html += summaryHtml
            ? `<div class="detail-card"><div class="detail-card-title">AI Visit Summary</div><div style="display:flex;flex-direction:column;gap:0.75rem;">${summaryHtml}</div></div>`
            : '';
    } else {
        html += `
...
function formatDate(isoString) {
    ...
    try {
        // SQLite stores timestamps without timezone — treat as UTC by appending Z
        const normalized = String(isoString).replace(' ', 'T').replace(/(\.\d+)?$/, s => s || '') + (isoString.includes('Z') || isoString.includes('+') ? '' : 'Z');
...
function formatDuration(startIso, endIso) {
    try {
        const normalize = s => String(s).replace(' ', 'T') + (String(s).includes('Z') || String(s).includes('+') ? '' : 'Z');
        const diffMs = new Date(normalize(endIso)) - new Date(normalize(startIso));
